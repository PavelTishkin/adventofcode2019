...
func TestGetSystemStateCycle(t *testing.T) {
	lines := []string{"<x=-1, y=0, z=2>",
		"<x=2, y=-10, z=-7>",
		"<x=4, y=-8, z=8>",
		"<x=3, y=5, z=-1>"}

	numSteps := calcSystemPeriod(lines)
	if numSteps != 2772 {
		t.Errorf("calcSystemPeriod = %d, expected 2772", numSteps)
	}

	// Doesn't work, returns 1.5 times expected answer
	...
	// numSteps = calcSystemPeriod(lines)
	// if numSteps != 4686774924 {
	// 	t.Errorf("calcSystemPeriod = %d, expected 4686774924", numSteps)
	// }
}
